...
export default {
  ...
  computed: {
    ...mapState('product', ['editProduct', 'editTranslation']),
    ...mapFields([
  ...
  methods: {
    ...mapMutations('product', ['addTranslation', 'setTranslation']),
    async save() {
      try {
        // Edit
        if (this.mode === 'edit') {
          const res = await this.$axios.put(
            '/api/products/translations/' + this.editTranslation.id,
        ...
        // Create
          ...
          const res = await this.$axios.post(
            `/api/products/translations/${this.editProduct.id}`,
            this.editTranslation
          )
