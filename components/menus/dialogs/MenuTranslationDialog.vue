...
export default {
  ...
  computed: {
    ...mapState('menu', ['editMenu', 'editTranslation']),
    ...mapFields([
  ...
  methods: {
    ...mapMutations('menu', ['addTranslation', 'setTranslation']),
    async save() {
      try {
        // Edit
        if (this.mode === 'edit') {
          const res = await this.$axios.put(
            '/api/menus/translations/' + this.editTranslation.id,
            this.editTranslation
          )
          this.setTranslation(res.data)
        }
        // Create
        else if (this.editMenu.id) {
          const res = await this.$axios.post(
            `/api/menus/translations/${this.editMenu.id}`,
            {
              ...this.editTranslation,
            }
          )
