<template>
  <v-container fluid>
    ...
    <v-row>
        ...
        <v-col cols="8">
            <total-orders-line :data="orders" :min="date[0]" :max="date[1]"></total-orders-line>
        </v-col>
    </v-row>

    <v-row>
        <v-col cols="6">
            <revenue-line :data="orders" :min="date[0]" :max="date[1]"></revenue-line>
        </v-col>
        <v-col cols="6">
            <v-card>
                <v-card-title>{{$t('top_products')}}</v-card-title>
                <v-card-text><top-product :min="date[0]" :max="date[1]" :data="orders"></top-product></v-card-text>
            </v-card>
        </v-col>
    </v-row>
  </v-container>
...
export default {
  name: 'AnalyticsPage',
  components: { DateRange, AnalyticsCard, OrderTypePie, TotalOrdersLine, RevenueLine, TopProduct },
  data() {
    return {
        orders: [],
        date: [],
  ...
  computed: {
    ...
    subEnd() {
        return {
            name: 'days_left',
            value: moment(this.activeSubscription.endAt).diff(moment(), 'days'),
            icon: 'mdi-clock'
        }
    }
  },
