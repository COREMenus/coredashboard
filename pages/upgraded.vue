<template>
  <v-container>
    <h1 class="font-weight-regular text-center">Subscription Upgraded Successfully</h1>
    <p class="text-center">Thank you for becoming a part of CoreMenus!</p>
    <v-row justify="center">
        <v-col cols="7">
            <v-card flat>
        <v-card-title class="text-weight-regular"> Your Subscription Details </v-card-title>
        <v-card-text>
            <v-list>
                <v-list-item>
...
export default {
  ...
  async created() {
    try {
        ...
        const res = await this.$axios.get(`/api/stripe/get-subscription/${this.$route.query.subscription_id}`)
